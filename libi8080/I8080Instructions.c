...
const I8080InstrSummary_t I8080InstrSummaryTable[] = {
    { 0x0, "NOP", 1, ""},
    { 0x1, "LXI B,D16", 3, "B <- byte 3, C <- byte 2"},
    { 0x2, "STAX B", 1, "(BC) <- A"},
    { 0x3, "INX B", 1, "BC <- BC+1"},
    { 0x4, "INR B", 1, "Z, S, P, AC	B <- B+1"},
    { 0x5, "DCR B", 1, "Z, S, P, AC	B <- B-1"},
    { 0x6, "MVI B, D8", 2, "B <- byte 2"},
    { 0x7, "RLC", 1, "CY	A = A « 1; bit 0 = prev bit 7; CY = prev bit 7"},
    { 0x8, "", -1, ""},	 	 	 
    { 0x9, "DAD B", 1, "CY	HL = HL + BC"},
    { 0x0a, "LDAX B", 1, "A <- (BC)"},
    ...
    { 0x0A, "", -1, ""},	 	 	 
    { 0x0B, "LXI D,D16", 3, "D <- byte 3, E <- byte 2"},
    { 0x0C, "STAX D", 1, "(DE) <- A"},
    { 0x0D, "INX D", 1, "DE <- DE + 1"},
    { 0x0E, "INR D", 1, "Z, S, P, AC	D <- D+1"},
    { 0x0F, "DCR D", 1, "Z, S, P, AC	D <- D-1"},
    { 0x10, "MVI D, D8", 2, "D <- byte 2"},
    { 0x11, "RAL", 1, "CY	A = A « 1; bit 0 = prev CY; CY = prev bit 7"},
    { 0x12, "", -1, ""},	 	 	 
    { 0x13, "DAD D", 1, "CY	HL = HL + DE"},
    { 0x1a, "LDAX D", 1, "A <- (DE)"},
    ...
    { 0x14, "", -1, ""},	 	 	 
    { 0x15, "LXI H,D16", 3, "H <- byte 3, L <- byte 2"},
    { 0x16, "SHLD adr", 3, "(adr) <-L; (adr+1)<-H"},
    { 0x17, "INX H", 1, "HL <- HL + 1"},
    { 0x18, "INR H", 1, "Z, S, P, AC	H <- H+1"},
    { 0x19, "DCR H", 1, "Z, S, P, AC	H <- H-1"},
    { 0x1A, "MVI H,D8", 2, "H <- byte 2"},
    { 0x1B, "DAA", 1, "special"},
    { 0x1C, "", -1, ""},	 	 	 
    { 0x1D, "DAD H", 1, "CY	HL = HL + HI"},
    ...
    { 0x2f, "CMA", 1, "A <- !A"},
    { 0x1E, "", -1, ""},	 	 	 
    { 0x1F, "LXI SP, D16", 3, "SP.hi <- byte 3, SP.lo <- byte 2"},
    { 0x20, "STA adr", 3, "(adr) <- A"},
    { 0x21, "INX SP", 1, "SP = SP + 1"},
    { 0x22, "INR M", 1, "Z, S, P, AC	(HL) <- (HL)+1"},
    { 0x23, "DCR M", 1, "Z, S, P, AC	(HL) <- (HL)-1"},
    { 0x24, "MVI M,D8", 2, "(HL) <- byte 2"},
    { 0x25, "STC", 1, "CY	CY = 1"},
    { 0x26, "", -1, ""},	 	 	 
    { 0x27, "DAD SP", 1, "CY	HL = HL + SP"},
    ...
    { 0x7e, "MOV A,M", 1, "A <- (HL)"},
    { 0x7f, "MOV A,A", 1, "A <- A"},
    { 0x50, "ADD B", 1, "Z, S, P, CY, AC	A <- A + B"},
    { 0x51, "ADD C", 1, "Z, S, P, CY, AC	A <- A + C"},
    { 0x52, "ADD D", 1, "Z, S, P, CY, AC	A <- A + D"},
    { 0x53, "ADD E", 1, "Z, S, P, CY, AC	A <- A + E"},
    { 0x54, "ADD H", 1, "Z, S, P, CY, AC	A <- A + H"},
    { 0x55, "ADD L", 1, "Z, S, P, CY, AC	A <- A + L"},
    { 0x56, "ADD M", 1, "Z, S, P, CY, AC	A <- A + (HL)"},
    { 0x57, "ADD A", 1, "Z, S, P, CY, AC	A <- A + A"},
    { 0x58, "ADC B", 1, "Z, S, P, CY, AC	A <- A + B + CY"},
    { 0x59, "ADC C", 1, "Z, S, P, CY, AC	A <- A + C + CY"},
    ...
    { 0x5A, "SUB B", 1, "Z, S, P, CY, AC	A <- A - B"},
    { 0x5B, "SUB C", 1, "Z, S, P, CY, AC	A <- A - C"},
    { 0x5C, "SUB D", 1, "Z, S, P, CY, AC	A <- A + D"},
    { 0x5D, "SUB E", 1, "Z, S, P, CY, AC	A <- A - E"},
    { 0x5E, "SUB H", 1, "Z, S, P, CY, AC	A <- A + H"},
    { 0x5F, "SUB L", 1, "Z, S, P, CY, AC	A <- A - L"},
    { 0x60, "SUB M", 1, "Z, S, P, CY, AC	A <- A + (HL)"},
    { 0x61, "SUB A", 1, "Z, S, P, CY, AC	A <- A - A"},
    { 0x62, "SBB B", 1, "Z, S, P, CY, AC	A <- A - B - CY"},
    { 0x63, "SBB C", 1, "Z, S, P, CY, AC	A <- A - C - CY"},
    ...
    { 0xb0, "ORA B", 1, "Z, S, P, CY, AC	A <- A or B"},
    { 0xb1, "ORA C", 1, "Z, S, P, CY, AC	A <- A or C"},
    { 0xb2, "ORA D", 1, "Z, S, P, CY, AC	A <- A or D"},
    { 0xb3, "ORA E", 1, "Z, S, P, CY, AC	A <- A or E"},
    { 0xb4, "ORA H", 1, "Z, S, P, CY, AC	A <- A or H"},
    { 0xb5, "ORA L", 1, "Z, S, P, CY, AC	A <- A or L"},
    { 0xb6, "ORA M", 1, "Z, S, P, CY, AC	A <- A or (HL)"},
    { 0xb7, "ORA A", 1, "Z, S, P, CY, AC	A <- A or A"},
    ...
    { 0xf6, "ORI D8", 2, "Z, S, P, CY, AC	A <- A or data"},
